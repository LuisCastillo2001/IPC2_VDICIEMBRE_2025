...
from lista_circular_doble_centros import ListaCircularDobleCentros
from lista_circular_simple_vms import ListaCircularSimpleVMs


def cargar_centros_datos(configuracion):
    """
    Carga los centros de datos del XML y los almacena en una lista circular doble
    
    Args:
        configuracion: Elemento XML con la configuración
        
    Returns:
        ListaCircularDobleCentros: Lista con los centros de datos
    """
    lista_centros = ListaCircularDobleCentros()
...
def cargar_maquinas_virtuales(configuracion):
    """
    Carga las máquinas virtuales del XML y las almacena en una lista circular simple
    
    Args:
        configuracion: Elemento XML con la configuración
        
    Returns:
        ListaCircularSimpleVMs: Lista con las máquinas virtuales
    """
    lista_vms = ListaCircularSimpleVMs()
...
def mostrar_menu():
    ...
    print("\n" + "="*60)
    print("  SISTEMA DE GESTIÓN - LISTAS CIRCULARES")
    print("="*60)
    print("1. Mostrar centros de datos (Lista Circular Doble)")
    print("2. Mostrar máquinas virtuales (Lista Circular Simple)")
    ...
    print("7. Agregar centro de datos")
    print("8. Agregar máquina virtual")
    print("9. Salir")
    print("="*60)
...
def agregar_centro(lista_centros):
    """Agrega un nuevo centro de datos"""
    print("\n--- Agregar nuevo centro de datos ---")
    id_centro = input("ID del centro: ").strip()
    ...
    lista_centros.agregar(centro)
    print(f"\n✓ Centro de datos '{nombre}' agregado exitosamente")


def agregar_vm(lista_vms):
    """Agrega una nueva máquina virtual"""
    print("\n--- Agregar nueva máquina virtual ---")
    id_vm = input("ID de la VM: ").strip()
    centro = input("Centro asignado: ").strip()
    so = input("Sistema operativo: ").strip()
    cpu = input("CPU: ").strip()
    ram = input("RAM: ").strip()
    almacenamiento = input("Almacenamiento: ").strip()
    ip = input("IP: ").strip()
    
    vm = MaquinaVirtual(id_vm, centro, so, cpu, ram, almacenamiento, ip)
    lista_vms.agregar(vm)
    print(f"\n✓ Máquina virtual '{id_vm}' agregada exitosamente")
...
def main():
    ...
    try:
        tree = ET.parse("Clase 7\\entrada.xml")
        root = tree.getroot()
        configuracion = root.find("configuracion")
        
        # Cargar datos en listas circulares
        lista_centros = cargar_centros_datos(configuracion)
        lista_vms = cargar_maquinas_virtuales(configuracion)
        
        print("\n¡Datos cargados exitosamente!")
        print(f"Centros de datos (Lista Circular Doble): {lista_centros.obtener_tamanio()}")
        print(f"Máquinas virtuales (Lista Circular Simple): {lista_vms.obtener_tamanio()}")
    ...
    while True:
        ...
        try:
            ...
            if opcion == "1":
                print("\n" + "="*60)
                print("    CENTROS DE DATOS (Lista Circular Doble)")
                print("="*60)
                ...
                print("\n" + "="*60)
                print("    MÁQUINAS VIRTUALES (Lista Circular Simple)")
                print("="*60)
                ...
                agregar_centro(lista_centros)
                
            elif opcion == "8":
                agregar_vm(lista_vms)
